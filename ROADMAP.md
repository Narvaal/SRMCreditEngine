...
### Passo 18 — Fechamento dos gaps apontados pela auto-avaliação _(concluído)_

Uma auto-avaliação honesta contra o enunciado ("onde um avaliador rigoroso vai bater") virou plano e execução no mesmo dia:

- **S10 fechado — Dependabot**: `.github/dependabot.yml` cobrindo os 6 manifestos (Gradle, npm ×2, Actions, Docker ×2), semanal.
- **E6 mitigado — teto no lote**: `@Size(max = 100)` em `LoteRecebivelRequest` — o processamento síncrono (uma transação por item) passa a ter latência limitada por design; acima disso, 400 com mensagem clara (coberto por teste). O caminho assíncrono (fila + resultado consultável) segue documentado como evolução, não implementado.
- **P5 fechado — teste de carga real** (`scripts/carga/`): seed de **1M de liquidações** via `generate_series` (segundos, set-based) + k6 em container (8 VUs, 60s) nas 3 consultas do requisito. **A primeira execução reprovou o sistema**: 40% de HTTP 500 e p95 de 2,8s no pior caso. Dois achados que nenhum teste funcional pegaria: o `/dev/shm` de 64MB (default do Docker) estourando com hash joins paralelos concorrentes do `NOT EXISTS` do extrato, e a FK `liquidacao_estornada_id` sem índice (anti-join com seq scan duplo e spill pra disco). Fixes: `shm_size: 256mb` no compose + índice parcial `idx_liquidacao_estornada` (migration V12). Depois: **0% de erro, p95 de 16/113/193ms, 115 req/s** — plano de execução virou Merge Anti Join com index-only scans (antes/depois completo em `scripts/carga/README.md`).
- **Cobertura medida — JaCoCo**: relatório gerado junto com `test` e publicado como artifact no CI — a pergunta "qual a cobertura?" passa a ter resposta com número (**87,9% de linhas / 89,7% de instruções** só nos testes de unidade/slice; os `*IT` somam em cima no CI), não contagem de testes.
- **Autenticação (S6) deliberadamente não implementada**: auth de verdade às vésperas da entrega seria o maior risco de regressão do projeto (9 suítes `@WebMvcTest` + fluxo de login no frontend), e auth de mentira (API key hardcoded) é pior sinal que gap documentado com plano — decisão registrada, defesa no próprio S6.
