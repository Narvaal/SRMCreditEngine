...
| S9 | As imagens Docker do sistema não rodam como root | ⚠️ Parcialmente atendido | `backend/Dockerfile` roda com usuário dedicado não-root desde `v1.0.1` (validado: `docker exec ... id` → `uid=999(app)`, `/actuator/health` `UP`) — corrigido como veículo real da simulação de gestão de crise do `ROADMAP.md` (Passo 12). `frontend/Dockerfile` ainda roda como root — bind de porta 80 exige root ou mudança de porta/`nginx.conf`/`docker-compose.yml`, mais invasivo, deixado como gap em aberto |
